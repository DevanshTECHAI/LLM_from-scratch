...
def get_batch(split, train_data, val_data):
    ...
    context_len = min(CONTEXT, len(data) - 1)
    if context_len < 1:
        raise ValueError("Encoded dataset is too small for training. Please regenerate the tokenizer/data with a larger corpus.")

    max_start = len(data) - context_len
    inds = torch.randint(0, max_start, (BATCH_SIZE,))
    x = torch.stack([data[i: i + context_len] for i in inds])
    y = torch.stack([data[i + 1: i + context_len + 1] for i in inds])
