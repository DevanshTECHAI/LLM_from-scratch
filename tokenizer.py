...
def train_tokenizer(input_file, model_prefix, vocab_size=4096):
    ...
    """
    # SentencePiece reserves a small number of special pieces (for example, byte fallback
    # and control tokens), so the requested vocabulary size must stay within the corpus's
    # practical limit. Clamp it to a safe value for small sample datasets.
    safe_vocab_size = min(int(vocab_size), 900)
